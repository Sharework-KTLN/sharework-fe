...
import { Card, Input, Select, Button, Row, Col, Image, Upload } from 'antd';
...
const CVManager = () => {
  ...
  useEffect(() => {
    const fetchUserSkills = async () => {
      ...
      try {
        ...
        if (!skillsRes.ok) {
          console.log('Không có kỹ năng nào cho người dùng');
          return;  // Dừng lại nếu không có dữ liệu
        }

        const skillsData = await skillsRes.json();
        setSelectedSkills(skillsData.map((item: Skill) => item.id)); // Lưu các ngành nghề đã chọn vào state
        ...
        setSkills(allSkillsData); // Lưu danh sách tất cả các ngành nghề vào state
      } catch (err: unknown) {
  ...
  useEffect(() => {
    const fetchUserMajors = async () => {
      ...
      try {
        ...
        setSelectedMajors(majorsData.map((item: Major) => item.id)); // Lưu các ngành nghề đã chọn vào state
        ...
        const allMajorsData = await allMajorsRes.json();
        setMajors(allMajorsData); // Lưu danh sách tất cả các ngành nghề vào state
      } catch (err: unknown) {
        if (err instanceof Error) {
  ...
  const handleFieldChange = <K extends keyof UserWithRelations>(
    field: K, 
    value: UserWithRelations[K]
  ) => {
    // Cập nhật thông tin người dùng trong Redux hoặc state
    // dispatch(updateUser({ ...user, [field]: value }));
    console.log(`${field} has been updated to:`, value);
  };

  const toggleEditable = () => {
    setIsEditable(prev => !prev); // Chuyển đổi trạng thái chỉnh sửa
  };
  ...
  return (
    <div style={{ width: '70%', margin: 'auto', padding: 20 }}>
      ...
      <Card
        style={{ marginTop: 20, border: "none", boxShadow: "none", padding:0 }}
      >
        <div style={{
          background: "linear-gradient(to right, #D4421E, #FFA07A)",
          padding: "16px",
          ...
          <Row gutter={16} style={{ marginBottom: 12 }}>
            ...
            <Col span={12}>
              ...
              <Select
                ...
                onChange={(value) => console.log("Selected majors:", value)}
              >
                {majors.map((major) => (
          ...
          <Row gutter={16} style={{ marginBottom: 12 }}>
            <Col span={24}>
              ...
              <Select
                ...
                onChange={(value) => console.log("Selected skills:", value)}
                >
                {skills.map((skill) => (
      ...
      >
        ...
        <div style={{
          ...
          <Row gutter={16} style={{ marginBottom: 12 }}>
            <Col span={24}>
              <label style={{ fontWeight: 500 }}>Thêm CV</label>
              <Input
